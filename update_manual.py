...
<head>
  ...
  <style>
    *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    body.edit-mode:not(.delete-mode) .page-img-wrap {{ cursor: crosshair; }}
    body.edit-mode.delete-mode .stamp {{
      pointer-events: auto; cursor: pointer;
    }}
    body.edit-mode.delete-mode .stamp:hover {{
    ...
    #editBar .mode-group {{ display: flex; gap: 4px; background: rgba(255,255,255,0.1); padding: 3px; border-radius: 6px; }}
    #editBar .mode-btn {{
...
<div id="editBar">
  <div class="mode-group">
    <button class="mode-btn active" id="modePlace" onclick="setEditSubMode('place')">📍 配置</button>
    <button class="mode-btn" id="modeDelete" onclick="setEditSubMode('delete')">🗑 削除</button>
  </div>
  <span class="edit-info" id="editInfo">画像をタップでスタンプ配置</span>
  <button onclick="saveStamps()">💾 保存</button>
...
var EDIT_SUB_MODE = 'place'; // 'place' or 'delete'
var GITHUB_EDIT_URL = 'https://github.com/KIMINORI14/hm-manual/edit/main/stamps.json';

function setEditSubMode(mode) {{
  EDIT_SUB_MODE = mode;
  var pb = document.getElementById('modePlace');
  var db = document.getElementById('modeDelete');
  if (pb) pb.classList.toggle('active', mode === 'place');
  if (db) db.classList.toggle('active', mode === 'delete');
  document.body.classList.toggle('delete-mode', mode === 'delete');
  var info = document.getElementById('editInfo');
  if (info) info.textContent = mode === 'place'
    ? '画像をタップでスタンプ配置'
    : '既存スタンプをタップで削除';
}}
  ...
  document.querySelectorAll('.page-block').forEach(function(block) {{
    ...
    stamps.forEach(function(s, idx) {{
      ...
      el.addEventListener('click', function(e) {{
        if (!EDIT_MODE || EDIT_SUB_MODE !== 'delete') return;
        e.stopPropagation();
...
function saveStamps() {{
  ...
  function proceed() {{
    var msg = '📋 クリップボードにコピーしました。\\n\\n' +
              'これからGitHubの編集画面を開きます。\\n' +
              '1. 既存の内容を全選択（Cmd+A / Ctrl+A）\\n' +
              '2. 貼り付け（Cmd+V / Ctrl+V）\\n' +
              '3. 画面下の「Commit changes」をクリック\\n\\n' +
              '※ログインが必要な場合はGitHubにログインしてください';
    alert(msg);
    window.open(GITHUB_EDIT_URL, '_blank');
  }}
  if (navigator.clipboard && navigator.clipboard.writeText) {{
    navigator.clipboard.writeText(text).then(proceed).catch(function() {{ showJsonFallback(text); }});
  }} else {{
...
function showJsonFallback(text) {{
  ...
  modal.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.85);z-index:9999;display:flex;flex-direction:column;padding:20px;';
  modal.innerHTML = '<div style="color:white;margin-bottom:10px;font-size:14px;">下のテキストをコピーして、開いたGitHubのエディタに貼り付けてください</div>' +
    '<textarea style="flex:1;width:100%;font-size:12px;padding:10px;font-family:monospace;"></textarea>' +
    '<div style="margin-top:10px;display:flex;gap:10px;">' +
    '<button onclick="window.open(GITHUB_EDIT_URL, \\'_blank\\')" style="background:#f39c12;color:white;border:none;padding:10px 20px;border-radius:6px;font-weight:700;">GitHubを開く</button>' +
    '<button onclick="this.closest(\\'div[style*=inset]\\').remove()" style="background:#7f8c8d;color:white;border:none;padding:10px 20px;border-radius:6px;">閉じる</button></div>';
